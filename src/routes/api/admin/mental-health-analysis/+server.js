...
export async function GET({ locals }) {
  ...
  try {
    ...
    const data = await statisticsService.getMentalHealthAnalysis();
    return json({
      success: true,
      ...data
    });
  } catch (error) {
    console.error('Admin mental health analysis error:', error);
    return json({ success: false, error: 'Failed to retrieve mental health analysis' }, { status: 500 });
  }
}
